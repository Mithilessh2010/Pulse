    ...
    <section className="border-y border-white/[0.06] bg-[#0E100F] py-24">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        <div className="grid gap-12 lg:grid-cols-[0.72fr_1.28fr] lg:items-end">
          ...
          <Reveal delay={0.08}>
            <motion.div
              className="overflow-hidden rounded-2xl border border-white/[0.08] bg-[#111412]"
              whileHover={{ y: -4, boxShadow: "0 28px 70px rgba(0,0,0,0.25)" }}
            ...
            >
              <div className="grid md:grid-cols-2">
                  ...
                  <div className="mt-4 space-y-2">
                    {oldStack.map((item, index) => (
                      <motion.div
                        ...
                        className="flex items-center justify-between rounded-lg border border-white/[0.06] bg-[#0B0D0C] px-3 py-2.5 text-sm text-[#8E958F]"
                      >
                ...
                <div className="p-5">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    {pulseStack.map((item, index) => (
                      <motion.div
                        ...
                        viewport={{ once: true }}
                        transition={{ delay: index * 0.055, duration: 0.42, ease: [0.16, 1, 0.3, 1] }}
                        whileHover={{ x: 2, borderColor: "rgba(47,125,104,0.38)" }}
                        className="flex items-center gap-2 rounded-lg border border-[#2F7D68]/20 bg-[#2F7D68]/[0.06] px-3 py-2.5 text-sm text-[#C8CCC7]"
                      >
                  ...
                  <motion.a
                    href="/product"
                    className="mt-5 inline-flex items-center gap-2 text-sm font-medium text-[#D5BC7A] transition-colors hover:text-[#E8D8AD]"
                    whileHover={{ x: 3 }}
                  >
                    See how the workspace fits together <ArrowRight className="h-4 w-4" />
                  </motion.a>
                </div>
              </div>
            </motion.div>
