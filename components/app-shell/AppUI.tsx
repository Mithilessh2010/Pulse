...
export function DashboardCard({
  ...
  return (
    <motion.section
      ...
      className={`rounded-xl border border-[var(--border-subtle)] bg-[var(--card-raised-bg)] p-4 ${className}`}
    >
      <div className="mb-4">
...
export function StatusBadge({ label }: { label: string }) {
  ...
  const color = risk ? "#F87171" : review ? "#FBBF24" : label.includes("Available") || label.includes("Approved") || label.includes("Completed") || label.includes("Low") ? "#D5BC7A" : "#4ADE80";
...
export function LoadingSpinner({ label = "Loading" }: { label?: string }) {
  return (
    <span className="inline-flex items-center gap-2 text-xs font-medium text-[var(--text-muted)]">
      <Loader2 className="h-3.5 w-3.5 animate-spin text-[#D5BC7A]" />
      {label}
    </span>
  );
}
...
export function EmptyState({ title, description }: { title: string; description: string }) {
  return (
    <div className="rounded-2xl border border-dashed border-[var(--border-subtle)] bg-[var(--card-bg)] p-6 text-center">
      <FileText className="mx-auto h-6 w-6 text-[#D5BC7A]" />
...
export function AskPulseCard({ answer = askFallback }: { answer?: string }) {
  ...
  return (
    <DashboardCard title="Ask Pulse" subtitle="Command assistant for workspace context">
        ...
        {["What is at risk?", "Who is overloaded?", "What needs approval?", "Write today's update"].map((chip) => (
          <button
            ...
            className={`rounded-full border px-3 py-1.5 text-xs transition ${
              activePrompt === chip
                ? "border-[#2F7D68]/50 bg-[#2F7D68]/15 text-white"
      ...
      <motion.div
        key={response}
        className="mt-4 rounded-xl border border-[#D5BC7A]/18 bg-[#D5BC7A]/[0.07] p-4"
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
      >
        <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-[#D5BC7A]">
      ...
      <div className="space-y-3">
        {items.map((item, index) => (
          <div key={item} className="flex gap-3">
            <div className="mt-1 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-white/[0.055] text-[11px] font-semibold text-[#D5BC7A]">{index + 1}</div>
